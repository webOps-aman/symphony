      ...
      <div className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex items-center text-sm text-gray-600">
            ...
            <span className="mx-2">&gt;</span>
            <span className="cursor-pointer hover:text-blue-600">{product.name}</span>
          </div>
        </div>
      </div>
                  ...
                  {images.map((img, idx) => (
                    <div
                      ...
                      className={`w-16 h-16 border-2 rounded-lg cursor-pointer overflow-hidden transition-all ${
                        selectedImage === idx ? 'border-blue-500 shadow-md' : 'border-gray-200 hover:border-gray-400'
                      }`}
                    >
          ...
          <div className="flex-1">
            <div className="bg-white rounded-lg p-6 mb-4">
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  <h1 className="text-xl font-normal text-gray-800 mb-3">{product.name}</h1>
                  <div className="flex items-center gap-4 mb-3 flex-wrap">
                    <div className="flex items-center gap-2 px-2 py-1 bg-green-600 text-white text-xs rounded">
                      <span className="font-semibold">4.4</span>
                      <span>★</span>
                    </div>
                    <span className="text-sm text-gray-600">
                      9,46,209 Ratings & 65,958 Reviews
                    </span>
                    <div className="flex items-center gap-1 text-green-600">
                      <span className="w-4 h-4">✓</span>
                      <span className="text-xs font-semibold">Assured</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* Available Offers */}
              <div className="mb-6">
                <h3 className="font-semibold text-base mb-3">Available offers</h3>
                  ...
                  {product.offers?.slice(0, 4).map((offer, idx) => (
                    <div key={idx} className="flex gap-2 text-sm">
                      ...
                      <div>
                        ...
                        <span className="text-gray-700"> {offer.replace('Bank Offer: ', '')}</span>
                      </div>
                    </div>
                  ))}
              ...
              {product.colors && product.colors.length > 0 && (
                <div className="mb-6">
                  <h3 className="font-medium text-sm text-gray-700 mb-3">Color</h3>
                    ...
                    {product.colors.map((color, idx) => (
                      <div
                        ...
                        className={`cursor-pointer transition-all ${
                          selectedColor === idx ? 'opacity-100' : 'opacity-60 hover:opacity-80'
                        }`}
                      >
            ...
            {product.specifications && (
              <div className="bg-white rounded-lg p-6 mb-4">
                <h3 className="font-semibold text-base mb-4">Specifications</h3>
                    ...
                    <div className="space-y-3 px-3">
                      {Object.entries(specs).map(([key, value]) => (
                        <div key={key} className="flex border-b border-gray-100 pb-2">
                          ...
                          <span className="w-3/5 text-sm text-gray-900 font-medium">{value}</span>
                        </div>
                      ))}
                    </div>
                  </div>
